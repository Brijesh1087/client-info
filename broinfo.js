const broInfo={};for(let x in navigator){if( navigator[x]=='string'){broInfo[x]=navigator[x]}else{for(let y in navigator[x]){if(typeof navigator[x][y]=='string'){broInfo[y]=navigator[x][y]}else{for(let z in navigator[x][y]){if(typeof navigator[x][y][z]=='string'){broInfo[z]=navigator[x][y][z]}}}}}};
let isp = {}
